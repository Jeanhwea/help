...
#+BEGIN_SRC python :preamble "# -*- coding: utf-8 -*-" :exports both :results file
  ...
  #rimg, gimg, bimg = cv.split(img)
  rimg, gimg, bimg = img[:, :, 0], img[:, :, 1], img[:, :, 2]

  # nrow, ncol, index
  plt.subplot(231), plt.imshow(subimg), plt.title('croped')
...
#+BEGIN_SRC python :preamble "# -*- coding: utf-8 -*-" :exports both :results file
  ...
  mask = np.zeros(lenna.shape).astype('uint8')
  cx, cy = int(lenna.shape[0] / 2), int(lenna.shape[1] / 2)
  mask = cv.circle(mask, (cx, cy), 250, (255, 255, 255), -1)

  # dst = 0.7*lenna + 0.3*mask + 0
  dst = cv.addWeighted(lenna, 0.7, mask, 0.3, 0)
  ...
  plt.subplot(224), plt.imshow(dst), plt.title('dst'), plt.xticks([]), plt.yticks([])
